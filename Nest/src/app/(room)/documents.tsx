...
export default function NotesScreen() {
  ...
  return <KeyboardDismissView><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.screen}>
    <Text style={styles.title}>Notes</Text>
    {!unlocked ? <>
      <Text style={styles.body}>Enter your four-digit Nest PIN to open your shared household note.</Text>
      <TextInput ref={pinInput} accessibilityLabel="Four digit PIN" autoFocus enterKeyHint="done" keyboardType="number-pad" maxLength={PIN_LENGTH} onChangeText={handlePinChange} onSubmitEditing={Keyboard.dismiss} placeholder="4-digit PIN" secureTextEntry style={styles.input} textContentType="oneTimeCode" value={pin} />
    ...
    <Modal animationType="slide" onRequestClose={() => !resetting && setResetVisible(false)} transparent visible={resetVisible}><KeyboardDismissView><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalBackdrop}><View accessibilityViewIsModal style={styles.modalCard}><View style={styles.modalHeader}><Text style={styles.modalTitle}>Reset Notes PIN</Text><Pressable disabled={resetting} onPress={() => { Keyboard.dismiss(); setResetVisible(false); }}><Text style={styles.close}>Close</Text></Pressable></View><ScrollView keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled"><Text style={styles.body}>Confirm your account password, then choose a new four-digit PIN.</Text><Text style={styles.label}>ACCOUNT PASSWORD</Text><TextInput autoFocus autoComplete="current-password" enterKeyHint="done" onChangeText={setPassword} onSubmitEditing={Keyboard.dismiss} placeholder="Your account password" secureTextEntry style={styles.input} textContentType="password" value={password} /><Text style={styles.label}>NEW FOUR-DIGIT PIN</Text><TextInput enterKeyHint="done" keyboardType="number-pad" maxLength={PIN_LENGTH} onChangeText={setNewPin} onSubmitEditing={Keyboard.dismiss} placeholder="••••" secureTextEntry style={styles.input} value={newPin} /><Text style={styles.label}>CONFIRM NEW PIN</Text><TextInput enterKeyHint="done" keyboardType="number-pad" maxLength={PIN_LENGTH} onChangeText={setConfirmPin} onSubmitEditing={() => void resetPin()} placeholder="••••" secureTextEntry style={styles.input} value={confirmPin} />{resetError ? <Text style={styles.error}>{resetError}</Text> : null}<Pressable disabled={resetting} onPress={() => void resetPin()} style={[styles.button, resetting && styles.disabled]}><Text style={styles.buttonText}>{resetting ? "Resetting…" : "Reset PIN"}</Text></Pressable></ScrollView></View></KeyboardAvoidingView></KeyboardDismissView></Modal>
  </KeyboardAvoidingView></KeyboardDismissView>;
}

const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: "#FFFCF5", padding: 24, paddingTop: 72 }, title: { color: "#24374B", fontFamily: "Georgia", fontSize: 34, fontWeight: "700", marginBottom: 12 }, body: { color: "#6B7785", lineHeight: 21, marginBottom: 20 }, input: { backgroundColor: "#FFFFFF", borderColor: "#E4E8E8", borderRadius: 14, borderWidth: 1, fontSize: 16, padding: 15, color: "#24374B" }, button: { alignItems: "center", backgroundColor: "#4975A8", borderRadius: 14, marginTop: 12, padding: 15 }, buttonText: { color: "#FFFFFF", fontWeight: "800" }, disabled: { opacity: 0.5 }, error: { color: "#B95048", fontWeight: "600", marginTop: 12 }, notice: { color: "#2D6C6A", fontWeight: "600", marginTop: 12 }, checking: { color: "#4975A8", fontWeight: "800", marginTop: 12 }, pinHint: { color: "#6B7785", fontSize: 13, marginTop: 12 }, resetLink: { alignSelf: "flex-start", marginTop: 16 }, resetText: { color: "#4975A8", fontSize: 14, fontWeight: "800" }, codeLabel: { color: "#6B7785", fontSize: 11, fontWeight: "800", letterSpacing: 1, marginBottom: 7 }, codeCard: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#F5D59C", borderRadius: 18, borderWidth: 1, flexDirection: "row", justifyContent: "space-between", padding: 17 }, codePressed: { backgroundColor: "#FFF3D8", transform: [{ scale: 0.98 }] }, codeText: { color: "#24374B", fontSize: 22, fontWeight: "900", letterSpacing: 2 }, copyText: { color: "#4975A8", fontSize: 10, fontWeight: "800", marginTop: 5 }, copyIcon: { color: "#4975A8", fontSize: 24, fontWeight: "800" }, noteHeader: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", marginBottom: 10, marginTop: 24 }, noteTitle: { color: "#24374B", fontFamily: "Georgia", fontSize: 23, fontWeight: "700" }, noteSubtitle: { color: "#6B7785", fontSize: 12, marginTop: 3 }, saveState: { color: "#2D6C6A", fontSize: 12, fontWeight: "800" }, noteInput: { backgroundColor: "#FFFFFF", borderColor: "#E4E8E8", borderRadius: 18, borderWidth: 1, color: "#24374B", flex: 1, fontSize: 17, lineHeight: 25, minHeight: 260, padding: 16 }, modalBackdrop: { backgroundColor: "rgba(36,55,75,.42)", flex: 1, justifyContent: "flex-end" }, modalCard: { backgroundColor: "#FFFCF5", borderTopLeftRadius: 26, borderTopRightRadius: 26, maxHeight: "88%", padding: 24 }, modalHeader: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", marginBottom: 10 }, modalTitle: { color: "#24374B", fontFamily: "Georgia", fontSize: 25, fontWeight: "700" }, close: { color: "#4975A8", fontWeight: "900" }, label: { color: "#6B7785", fontSize: 11, fontWeight: "800", letterSpacing: 1, marginBottom: 7, marginTop: 14 },
});
