...
import { ActivityIndicator, Alert, Keyboard, KeyboardAvoidingView, Modal, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
...
function HomeContent({ nestJoinCode, refreshRoom, regenerateJoinCode, room, userId }: { nestJoinCode: string | null; refreshRoom: () => Promise<void>; regenerateJoinCode: () => Promise<string>; room: RoomSnapshot; userId: string }) {
  ...
  return <View style={styles.screen}>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl onRefresh={() => void refreshAll()} refreshing={loading} tintColor="#28634E" />}>
      <View style={styles.header}><View><Text style={styles.eyebrow}>YOUR NEST</Text><Text style={styles.title}>{room.room.name}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Settings" onPress={() => router.push("/profile")} style={styles.profileButton}><Text style={styles.profileText}>Settings</Text></Pressable></View>
