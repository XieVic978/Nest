...
const initialChores: Chore[] = [
  { id: "trash", title: "Take out trash", description: "Bins go to the curb.", assignee: "Alex", due: "2026-09-04", status: "Active", priority: "Normal", recurrence: "Weekly", rotation: ["Alex", "Maya"] },
  { id: "groceries", title: "Weekly grocery run", description: "Restock household basics.", assignee: "Maya", due: "2026-09-06", status: "Upcoming", priority: "Normal", recurrence: "Weekly", rotation: ["Alex", "Maya"] },
  { id: "bathroom", title: "Clean bathroom", description: "Quick weekly clean.", assignee: "Maya", due: "2026-09-25", status: "Upcoming", priority: "High", recurrence: "Every 2 weeks", rotation: [] },
  { id: "counters", title: "Wipe kitchen counters", description: "", assignee: "Jordan", due: "2026-09-03", status: "Completed", priority: "Low", recurrence: "Once", rotation: [], completedBy: "Jordan", completedAt: "Sep 3, 4:30 PM" },
];
...
function nextDueDate(due: string, recurrence: Recurrence) {
  const parsed = dueAsDate(due) ?? new Date();
...
function displayedStatus(chore: Chore): Status {
  if (chore.status === "Completed" || chore.recurrence === "Once") return chore.status;
...
function fairestAssignee(rotation: string[], chores: Chore[], fallback: string) {
  if (rotation.length < 2) return fallback;
  const counts = rotation.reduce<Record<string, number>>((current, member) => ({ ...current, [member]: 0 }), {});
  chores.filter((chore) => chore.status !== "Completed" && sameRoommateGroup(chore.rotation, rotation)).forEach((chore) => { if (chore.assignee in counts) counts[chore.assignee] += 1; });
...
export default function ChoresScreen() {
  ...
  const openForm = (chore?: Chore) => {
    if (chore) { setEditingId(chore.id); setTitle(chore.title); setDescription(chore.description); setAssignee(chore.assignee); setDue(chore.due); setRecurrence(chore.recurrence); setPriority(chore.priority); setRotation(chore.rotation); setRotationEnabled(chore.rotation.length > 1); }
    else { setEditingId(null); setTitle(""); setDescription(""); setAssignee("Alex"); setDue("2026-09-12"); setRecurrence("Once"); setPriority("Normal"); setRotation([]); setRotationEnabled(false); }
    setShowForm(true);
  };

  const saveChore = () => {
    if (!title.trim()) return;
    const activeRotation = rotationEnabled ? rotation : [];
    setChores((current) => {
      const fairAssignee = fairestAssignee(activeRotation, current.filter((item) => item.id !== editingId), assignee);
      const chore: Chore = { id: editingId ?? Date.now().toString(), title: title.trim(), description: description.trim(), assignee: fairAssignee, due, status: "Upcoming", priority, recurrence, rotation: activeRotation };
      return editingId ? current.map((item) => item.id === editingId ? { ...item, ...chore, status: item.status, completedBy: item.completedBy, completedAt: item.completedAt } : item) : [chore, ...current];
    });
    setShowForm(false);
  };
  ...
  const completeChore = (chore: Chore) => {
    ...
    setChores((current) => {
      const remaining = current.filter((item) => item.id !== chore.id);
      const nextAssignee = fairestAssignee(chore.rotation, remaining, chore.assignee);
      ...
      return [completed, ...(next ? [next] : []), ...remaining];
    });
  };

  const deleteChore = (id: string) => setChores((current) => current.filter((chore) => chore.id !== id));
  const confirmDelete = (chore: Chore) => Alert.alert("Delete chore?", `Delete “${chore.title}”? This cannot be undone.`, [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: () => deleteChore(chore.id) }]);
