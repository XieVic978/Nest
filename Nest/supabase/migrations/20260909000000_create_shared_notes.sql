...
begin
  ...
  if tg_op = 'INSERT' then
    if new.created_by <> auth.uid() or new.updated_by <> auth.uid() then
      raise exception 'invalid_note_author';
    end if;
  else
    if new.room_id <> old.room_id or new.created_by <> old.created_by
       or new.created_at <> old.created_at then
      raise exception 'note_ownership_immutable';
    end if;
...
create trigger validate_shared_note_trigger
before insert or update on public.shared_notes
for each row execute function private.validate_shared_note();

drop policy if exists "members can read shared notes" on public.shared_notes;
create policy "members can read shared notes"
...
create policy "members can create shared notes"
...
create policy "members can edit shared notes"
...
create policy "members can delete shared notes"
